...
function update() {
  ...
  const currRoute = useSelector((state)=>state.route.getOne.data)
  ...
  useEffect(() => {
    if (!id) return;
    dispatch(getOneRouteAction(id))
  }, [id]);
  useEffect(() => {
    if(!currRoute) return
    setRouteName(currRoute.routeName)
    setRouteStations(currRoute.stops)
  }, [currRoute])
